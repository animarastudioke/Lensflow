...
export function EditQuoteForm({ studioSlug, clients, initialValues }: EditQuoteFormProps) {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        <Link
          href={`/dashboard/${studioSlug}/quotes/${initialValues.id}`}
          className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-4"
        >
          <ArrowLeft className="h-4 w-4" />
          Back to quote
        </Link>
        <h1 className="text-display-md font-display font-semibold text-foreground">Edit Quote</h1>
      </div>
      ...
      <form onSubmit={onSubmit}>
        <Card>
          ...
          <CardContent className="space-y-6">
            <div className="space-y-2">
              ...
              <Input id="title" name="title" defaultValue={initialValues.title} required maxLength={150} />
            </div>
        ...
        <div className="flex items-center gap-3 mt-6">
          ...
          <Button type="button" variant="outline" asChild>
            <Link href={`/dashboard/${studioSlug}/quotes/${initialValues.id}`}>Cancel</Link>
          </Button>
        </div>
      </form>
    </div>
